...
impl Codegen<'_> {
    /// Print a quoted JSX attribute, preserving existing entity spelling.
    pub(crate) fn print_jsx_attribute_string(&mut self, value: JSStr<'_>) {
        if let Some(value) = value.as_str() {
            ...
            self.print_wtf8_jsx_attribute_string(value);
        }
    }
...
impl Codegen<'_> {
    #[cold]
    fn print_wtf8_jsx_attribute_string(&mut self, value: JSStr<'_>) {
        let quote = if value.chars().any(|ch| ch.to_char() == Some('"')) { b'\'' } else { b'"' };
        self.print_ascii_byte(quote);
        for ch in value.chars() {
            match ch.to_char() {
                // Synthetic attributes can contain both quote characters.
                Some('\'') if quote == b'\'' => self.print_str("&apos;"),
                Some(ch) => self.code.print_char(ch),
                None => {
                    // A lone surrogate cannot be written directly as UTF-8.
                    // Emit a JSX character reference that decodes to the same UTF-16 code unit.
...
///
/// On 64-bit platforms the 16 function pointers occupy a pair of L1 cache lines.
