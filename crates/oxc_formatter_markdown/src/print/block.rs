...
pub enum Parent {
    ...
    /// `after_checkbox`: the block follows a task checkbox on its line.
    ListItem {
        loose: bool,
        after_checkbox: bool,
    },
...
fn double_gap(
    ...
    // The rest of a paragraph a definition was split from,
    // whose first line would open a block at a block start (`<Badge />`):
    // kept on the definition's next line, a continuation as in the source
    let remainder_after_definition = || {
        matches!((previous, node),
            (Block::Definition(_), Block::Paragraph(p)) if inline::first_line_opens_block(&p.children, f))
    };
    ...
    !(is_sibling_node
        ...
        || prev_is_ignore
        || (adjacent
            && (html_after_html || html_after_paragraph || liquid || remainder_after_definition())))
}
...
fn write_html_block<'a>(
    ...
    // micromark keeps the closing line ending as content in a few shapes (`HtmlBlock::trailing_newline`);
    // Prettier prints it, and the block gains a line on every pass. It is not printed.
    write!(
        f,
        mark_as_root(&format_with(|f| write_lines(value.split('\n'), literal_line_break(), f)))
    );
}
...
fn write_math_block<'a>(math: &'a MathBlock<'a>, f: &mut MarkdownFormatter<'_, 'a>) {
    ...
    if !math.lines.is_empty() {
        ...
        // As for code: a trailing blank line must survive
        // (`hard_line_break` would print nothing after it)
        write!(f, exact_line_breaks(1));
    }
    write!(f, token("$$"));
}
