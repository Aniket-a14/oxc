...
pub struct MarkdownFormatContext<'a> {
    ...
    /// its fence must avoid them.
    code_span_literal_runs: RefCell<Vec<(u32, u64)>>,
    /// The current phrasing content's opaque source spans
    /// (code spans, HTML, autolinks, math, liquid, link targets), sorted;
    /// `true` marks code spans. Wrapping never cuts inside one.
...
pub enum Raw {
    ...
    Text,
    /// On a dialect shape line, where emphasis markers stay as written too.
    Line,
}
...
impl<'a> MarkdownFormatContext<'a> {
    ...
    pub fn raw_text(&self) -> &Cell<Raw> {
        &self.raw_text
    }

    pub fn code_span_literal_runs(&self) -> &RefCell<Vec<(u32, u64)>> {
        &self.code_span_literal_runs
    }

    pub fn opaque_spans(&self) -> &RefCell<Vec<(Span, bool)>> {
        &self.opaque_spans
    }
