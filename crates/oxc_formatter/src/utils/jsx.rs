...
/// Whether the children of this element are read as raw `JSXText` by a compile-time DSL
/// instead of going through the JSX whitespace rules.
///
/// JSX drops whitespace containing a newline between text and an element,
...
///
/// Matches Prettier: only `<fbt>` is recognized, and only its direct children are affected.
