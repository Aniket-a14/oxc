    ...
    if property_name.eq("mockReturnValue") || property_name.eq("mockReturnValueOnce") {
        ...
        match expr {
            ...
            Expression::FunctionExpression(func_expr) => {
                ...
                report(is_once, property_span, Some(func_expr.span), arg_expr, ctx);
            }
            _ => (),
        }
    }
...
fn report<'a>(
    ...
    let mock_promise_resolve = if is_once { "mockResolvedValueOnce" } else { "mockResolvedValue" };
    let mock_promise_reject = if is_once { "mockRejectedValueOnce" } else { "mockRejectedValue" };
    ...
    if call_expr.arguments.len() <= 1 {
        ctx.diagnostic_with_fix(
            ...
            |fixer| {
                let content = fix(fixer, prefer_name, call_expr);
